...
#plt.rcParams["ytick.linewidth"]=2
...
def Run_Simulations():
    ...
    force_init=30e-6;
    for i in numpy.arange(3,7,0.5):
        dir_temp=str(int(10*i))
        os.mkdir(dir_temp)
        subprocess.call(['cp','train.py',dir_temp+"/"])
        os.chdir(dir_temp)
        subprocess.call(['./train.py','--force='+str(force_init),'--batch','--every='+str(50000),'--max_iters=200001',\
        '--iwidth=10','--output=train','--start='+str(375-int(i*50)),'--finish='+str(375+int(i*50)),'--output_format=vtk','--bc_wall_grad_order=1'])
        os.chdir("..")
...
def Compare_Plots():
    ...
    #read the giavedoni data (not precise though)
    heil=genfromtxt("../../Python/heil.csv",delimiter=';',dtype=float)
    ...
    #labels=[r'''$H_{eff}='''+str(value-2)+r'''$''' for value in ny]
    leg=pylab.legend(["Heil","This work"],fancybox=True)
    ...
    pylab.savefig("capillaries_comparison_train.eps",format="EPS",dpi=300)
    pylab.show()
