...
class GeoFE(geo.LBMGeo3D):
    ...
    def init_fields(self):
        ...
        start=self.sim.options.start
        finish=self.sim.options.finish
        ...
        self.sim.phi[np.logical_and(np.logical_and(
                       ...
                       np.logical_and(hx >= start, hx<=finish))] = -1.0
        self.sim.rho[:] = 1.0
...
class FESim(lbm.BinaryFluidFreeEnergy):
    ...
    def __init__(self, geo_class, defaults={}):
        ...
        opts = []
        opts.append(optparse.make_option('--force', dest='force', type='float', default=30e-6))
        opts.append(optparse.make_option('--iwidth', dest='iwidth', type='int', default=10))
        opts.append(optparse.make_option('--start',dest='start',type='int',default=250))
        opts.append(optparse.make_option('--finish',dest='finish',type='int',default=500))
