...
class GeoFE(geo.LBMGeo3D):
    ...
    def define_nodes(self):
        ...
        wall_map = np.logical_or(np.logical_or(hy < self.wall_width, hy >= self.lat_ny-self.wall_width),
                                  np.logical_or(hx<self.wall_width,hx>=self.lat_nx-self.wall_width))
    ...
    def init_fields(self):
        ...
        self.sim.phi[np.logical_and(np.logical_and(
                       np.logical_and(hx >= width-1 + self.wall_width, hx <= self.lat_ny-width-self.wall_width),
                       np.logical_and(hy >= width-1 + self.wall_width, hy <= self.lat_ny-width-self.wall_width)),
                     np.logical_and(hz>=self.lat_nz/3.0,hz<=2*self.lat_nz/3.0))] = -1.0
        self.sim.rho[:] = 1.0
...
class FESim(lbm.BinaryFluidFreeEnergy):
    ...
    def __init__(self, geo_class, defaults={}):
        settings = {'verbose': True, 'lat_nx': 32,'grid': 'D3Q19',
                    'lat_ny': 32, 'lat_nz' :128 ,
                    'kappa': 0.04, 'Gamma': 10.0, 'A': 0.04,
                    'scr_scale': 1,
